# 47 — Pipeline cancellation: Cancelled outcome, distinguished from failures

Status: ready-for-agent

## Parent

New feature request (overlay cancel button — this session). Foundation slice for issue 48.
...
refiner and injector — but its `catch` blocks currently swallow **everything**
...
Crucially, real transcription/refiner failures must STILL map to their existing outcomes —
cancellation must not become a catch-all that masks genuine errors.

## Acceptance criteria

- [ ] `DictationOutcome.Cancelled` exists.
- [ ] A transcriber throwing `OperationCanceledException` → `RunAsync` returns `Cancelled`; the injector is never called. Unit-tested (RED first).
- [ ] A refiner throwing `OperationCanceledException` → `Cancelled` (not `RefinedOffline`); nothing injected. Unit-tested.
